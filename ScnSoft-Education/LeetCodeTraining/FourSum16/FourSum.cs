namespace LeetCodeTraining.FourSum16
{
    public class Solution
    {
        public IList<IList<int>> FourSum(int[] nums, int target)
        {
            ...
            {
                if (firstNumIndex > 0 && nums[firstNumIndex] == nums[firstNumIndex - 1]) continue;
                for (int secondNumIndex = firstNumIndex + 1; secondNumIndex < nums.Length - 2; secondNumIndex++)
                {
                    if (secondNumIndex > firstNumIndex + 1 && nums[secondNumIndex] == nums[secondNumIndex - 1]) continue;
